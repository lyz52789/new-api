...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if _, err := model.CreateAffiliateCommissionForTopUp(topUp); err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 创建推广分佣失败 trade_no=%s user_id=%d client_ip=%s error=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), err.Error()))
			}
			appmetrics.RecordTopupSuccess(topUp.PaymentMethod, "epay", decimal.NewFromFloat(topUp.Money).Mul(decimal.NewFromInt(100)).Round(0).IntPart())
